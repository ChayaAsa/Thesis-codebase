...
DEFAULT_CAL = HERE / "FT44764" / "FT44764.cal"
...
class ftsensor:
    ...
    def __init__(self, calibration_file_path=None, tool_offset=[0,0,0,0,0,0]):
        if calibration_file_path is None:
            calibration_file_path = str(DEFAULT_CAL)
    ...
    def reBias(self, duration=2):
        current_time = time.time()

        values = []
        while time.time()< current_time + duration:
            values.append(np.array(self.read_avg_voltage()))
