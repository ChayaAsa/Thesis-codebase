...
def _control_worker(bus: MotorBus, motor: MITMotor,
                    model, norm, device: torch.device,
    ...
    try:
        ...
        print(
            ...
            f"  NN available : {'yes' if model is not None else 'no (model not found)'}\n"
        ...
        try:
            while not Terminate.is_set():
                ...
                f_nn_raw  = nn_infer(model, norm, tau_meas, pos, vel, device) \
                            if model is not None else f_ati_raw
...
def main() -> None:
    ...
    model, norm = load_model(MODEL_PATH, device)
    ...
    plotter = PlotThread(nn_available=(model is not None))
    ...
    ctrl_thread = threading.Thread(
        ...
        args=(bus, motor, model, norm, device, plotter, logger),
