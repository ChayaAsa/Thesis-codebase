...
Scale factor for ramping actuator output during soft-stop. Returns `1.0`
during normal run, then decreases linearly to `0.0` over the configured fade
window. Multiply your torque or velocity command by this:
...
A second Ctrl+C during fade triggers a hard stop.

### `stop()`

Stop the loop on the next iteration from inside the loop body.
...
Seconds since the loop started. Returns `0.0` before the first iteration.

### `close()`

Restore prior signal handlers. Call this if you `break` out of the loop
manually instead of letting it exhaust.

```python
loop = RealtimeLoop(dt=0.02)
try:
    for t in loop:
        if done:
            break
