...
ADM_IMP_EXTRA_COLUMNS = [
    ...
    'kp1', 'kp2', 'kp3', 'kd1', 'kd2', 'kd3', 'b_adm', 'k_spring', 'lpf_alpha', 'vel_limit',
]
...
# ── Admittance model  B_adm * xdot_des = F_err_vec − K_spring * x_err ────────
B_ADM       = 10.0    # virtual damping [N·s/m]
                       # Start high (safe, slow) then reduce for faster response.
K_SPRING    = 1.0     # task-space spring [N/m]
                       # Set nonzero so the arm returns to home when F_des=0.
...
# Safety
VEL_LIMIT   = 1.0     # max |qdot_des| per joint [rad/s]
TORQUE_LIMIT = 3.0    # only used for console display (motor hard limit)
    ...
    while not Terminate.is_set():
        try:
            ...
            if 'not yet been acquired' in msg or 'samples requested' in msg:
                time.sleep(0.02)   # NI-DAQ buffer not ready yet; retry
                continue
...
# Control worker
_TIMEOUT_ABORT = 5   # consecutive CAN timeouts before aborting setup
...
def setup(motors: list[MITMotor], sensor: ftsensor) -> bool:
    try:
        ...
        # Zero motors here — from now on motor reports 0 at this physical pose
        for m in motors:
        ...
        print(
            f"Config: F_des={F_DES_N}N  B_adm={B_ADM}  K_spring={K_SPRING}\n"
...
def loop(motors: list[MITMotor],
         ...
         ln_qdes: list[Line], ln_qmeas: list[Line], ln_qdot: list[Line]) -> None:
    try:
        # q_des starts at init_q so the arm holds its pose when control begins
        q_des  = INIT_Q.copy()   # integrated virtual joint position
        ...
        try:
            for t in rt_loop:
                ...
                # First-order: B * xdot_des = F_err_vec [- K * x_err_vec]
                # F_err_vec is the force error projected onto PUSH_DIR
                xdot_des = PUSH_DIR * f_err / b_adm   # [m/s] (3,)

                # Optional spring: pull q_des back toward home (q_des=0) in
                # task space using Jacobian to convert Cartesian spring to joint
                if k_spring > 1e-12:
                    # Cartesian spring restoring velocity: -k_spring/b * x_dev
                    p_ee   = dyn.evaluate_fk(q)['ee_position']
                    p_home = dyn.evaluate_fk(INIT_Q)['ee_position']
                    x_err  = p_ee - p_home           # deviation from home [m]
                    xdot_des -= (k_spring / b_adm) * x_err

                # ── Joint velocity (q2, q3 only — q1 locked) ────────────────
                Jv      = dyn.evaluate_jacobian(q)[:3, :]   # 3×3 linear-velocity rows
                # q1 (base yaw) is permanently locked at INIT_Q[0].
                # Use lstsq on the 3×2 sub-Jacobian [q2, q3] with the full
                # xdot_des = [v_push, 0, 0].  This constrains BOTH x≈v_push
                # AND z≈0, so the EE approaches the wall in a straight line.
                # The 1-D projection (Jv.T@PUSH_DIR) only constrained x, leaving
                # a z side-component that made the arm "go sideways".
                Jv_23            = Jv[:, 1:]                  # 3×2: q2 and q3 columns
                ...
                # JOINT_LIMITS are deltas from INIT_Q, so add INIT_Q before clipping
                ...
                if logger is not None:
                    logger.log(
                        ...
                        b_adm=b_adm,
                        k_spring=k_spring, lpf_alpha=alpha, vel_limit=vel_limit,
                    )
...
def main() -> None:
    ...
    # Settle hardware BEFORE enabling MIT mode so the firmware watchdog never
    # fires during the sequential enable calls or PlotThread/DataLogger setup.
    bus.drain()
    time.sleep(0.5)
    ...
    logger  = DataLogger('adm_imp', ADM_IMP_EXTRA_COLUMNS, directory=HERE)
    # logger = None
