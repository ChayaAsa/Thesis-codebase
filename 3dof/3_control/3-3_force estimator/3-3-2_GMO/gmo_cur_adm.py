...
GMO_CUR_ADM_EXTRA_COLUMNS = [
    'f_des_N', 'f_gmo_N', 'f_cur_N', 'f_sensor_N',
    'r1_Nm', 'r2_Nm', 'r3_Nm',
    ...
    'k_obs', 'tau_lpf_alpha', 'b_adm', 'k_spring',
...
# ── Current pre-filter (applied to measured motor torque before the GMO) ──────
TAU_LPF_ALPHA = 0.3
...
def setup(motors: list[MITMotor], sensor: ftsensor) -> bool:
    try:
        ...
        print("Step 3: Press Enter to START current-driven GMO admittance control.")
        kb = KeyboardLine()
        while kb.poll() is None and not Terminate.is_set():
            cmd_lock(motors)
            time.sleep(dt_boot)
        print(
            f"\nConfig: K_obs={K_OBS} rad/s  τ_LPF_α={TAU_LPF_ALPHA}\n"
...
def loop(motors: list[MITMotor],
         ...
         ln_fdes: Line, ln_fgmo: Line, ln_fcur: Line, ln_fsen: Line,
    ...
    try:
        ...
        # GMO state
        M0, _, G_init = dyn.evaluate_MCG(INIT_Q, np.zeros(3))
        p0           = M0 @ np.zeros(3)
        gmo_integral = np.zeros(3)
        r_gmo        = np.zeros(3)
        # Warm-start the measured-torque LPF at gravity: at rest tau_m ≈ G and
        # beta = C^T·qdot - G = -G, so tau_m + beta ≈ 0 and r starts near zero.
        ...
        try:
            for t in lp:
                ...
                if plotter.params.pop('reset', False):
                    ...
                    q_des[:]         = INIT_Q.copy()
                    qdot_des[:]      = 0.0
                    M0r, _, G0r      = dyn.evaluate_MCG(INIT_Q, np.zeros(3))
                    p0               = M0r @ np.zeros(3)
                    tau_meas_filt[:] = G0r
                    f_sen_filt       = FORCE_SIGN * ft_latest[FORCE_AXIS]
                ...
                # ── Measured actuator torque (DH frame), low-pass filtered ─
                ...
                # ── GMO update (driven by MEASURED torque, not tau_cmd) ────
                beta = C.T @ qdot - G
                p    = M @ qdot

                gmo_integral += dt * (tau_meas_filt + beta + r_gmo)
                r_gmo = k_obs * (p - p0 - gmo_integral)

                # Cartesian force estimate
                # With measured torque as input, r_gmo converges to the joint
                # reaction torque; the externally-applied torque is -r_gmo,
                # which equals the direct current formula tau_m - C·qdot - G.
                tau_ext = -r_gmo
                ...
                # ── Raw current-only estimate (no observer, for comparison) ─
                tau_ext_cur = tau_meas_filt - C @ qdot - G
                if cond < 1e6:
                    F_cart_cur = np.linalg.solve(Jv.T, tau_ext_cur)
                else:
                    F_cart_cur = np.linalg.pinv(Jv.T) @ tau_ext_cur
                f_cur_scalar = float(PUSH_DIR @ F_cart_cur)
                ...
                for i in range(3):
                    ...
                    ln_qmeas[i].push(q[i])
                _src = 'ATI' if use_ati else 'GMO(cur)'
                plotter.info = [
                ...
                if logger is not None:
                    logger.log(
                        ...
                        f_des_N=f_des, f_gmo_N=f_gmo_scalar, f_cur_N=f_cur_scalar,
                        f_sensor_N=f_sen_filt,
                        ...
                        k_obs=k_obs, tau_lpf_alpha=tau_lpf_alpha,
                ...
                if t - _last_print >= 0.2:
                    ...
                    print(
                        f"t={t:7.3f}s  "
                        f"GMO={f_gmo_scalar:+.3f}N  Curr={f_cur_scalar:+.3f}N  "
                        f"ATI={f_sen_filt:+.3f}N  des={f_des:+.2f}N  "
...
def main() -> None:
    ...
    ln_fgmo = Line(color='red',     label='GMO(cur) est.', lw=1.5)
    ln_fcur = Line(color='green',   label='Current raw',   lw=1.0, alpha=0.6)
    ...
    plotter = PlotThread(
        title='3-DOF Current-Driven GMO Admittance Control',
        on_close=Terminate.set,
        on_reset=_on_reset,
        on_stop=Terminate.set,
    )

    plotter.plot(1).set([ln_fdes, ln_fgmo, ln_fcur, ln_fsen])
    ...
    logger = DataLogger('gmo_cur_adm', GMO_CUR_ADM_EXTRA_COLUMNS, directory=HERE)
    ...
    ctrl_thread = threading.Thread(
        ...
        args=(motors, plotter, logger,
              ln_fdes, ln_fgmo, ln_fcur, ln_fsen, ln_r, ln_qdes, ln_qmeas),
