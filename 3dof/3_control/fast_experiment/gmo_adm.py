...
GMO_ADM_EXTRA_COLUMNS = [
    ...
    'k_obs', 'm_adm', 'xdot_alpha', 'b_adm', 'k_adm',
...
def setup(motors: list[MITMotor], sensor: ftsensor) -> bool:
    try:
        ...
        while kb.poll() is None and not Terminate.is_set():
            ...
            time.sleep(dt_boot)
        print(
            f"\nConfig: K_obs={K_OBS} rad/s  B_adm={B_ADM}  K_spring={K_ADM}\n"
...
def loop(motors: list[MITMotor],
         ...
         ln_r: list[Line], ln_qdes: list[Line], ln_qmeas: list[Line],
            ...
            for t in lp:
                ...
                if plotter.params.pop('record_trigger', False):
                    ...
                    logger_box[0] = DataLogger(
                        'gmo_adm', GMO_ADM_EXTRA_COLUMNS, directory=HERE)
                    signal_gen.reset()
                    record_start_t = t
                    signal_start_t = t
                    recording      = True
                ...
                if logger_box[0] is not None:
                    logger_box[0].log(
                        ...
                        k_obs=k_obs, m_adm=m_adm, xdot_alpha=xdot_alpha, b_adm=b_adm, k_adm=k_adm,
