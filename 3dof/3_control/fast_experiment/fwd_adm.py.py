...
FWD_ADM_EXTRA_COLUMNS = [
    ...
    'kf', 'tau_lpf_alpha', 'm_adm', 'xdot_alpha', 'b_adm', 'k_adm',
...
# Cosine: f = AMPLITUDE * sin(2π·FREQ·t)  — starts at 0, no spike
COS_AMPLITUDE = 8.0         # N peak amplitude
COS_FREQ_HZ   =  0.2        # Hz

# Random step: uniform force levels & hold durations, fixed seed → identical
# across all methods so results are directly comparable.
STEP_LO      =  0.0         # N minimum force level
STEP_HI      = 8.0         # N maximum force level
STEP_DUR_LO  =  1.0         # s minimum hold time
STEP_DUR_HI  =  5.0         # s maximum hold time
RANDOM_SEED  =   42         # fixed

# Time to hold F_des = 0 at the start of every recording (settle / zero-ref)
RECORD_ZERO_S     = 5.0     # s
...
class SignalGenerator:
    ...
    def __init__(self) -> None:
        self._steps: list[tuple[float, float]] = []   # (force_N, end_time_s)
        self.reset()
    ...
    def reset(self) -> None:
        if SIGNAL_MODE == 'random_step':
            ...
            self._steps = []
            t = 0.0
            while t < 600.0:               # pre-generate 10 min
    ...
    def get(self, t_sig: float) -> float:
        if SIGNAL_MODE == 'cos':
            # (COS_AMPLITUDE/2)*(1-cos) → starts at 0, rises to COS_AMPLITUDE at half period
...
def loop(motors: list[MITMotor],
         ...
         ln_tau: list[Line], ln_qdes: list[Line], ln_qmeas: list[Line],
            ...
            for t in lp:
                ...
                if plotter.params.pop('record_trigger', False):
                    ...
                    logger_box[0] = DataLogger(
                        'fwd_adm', FWD_ADM_EXTRA_COLUMNS, directory=HERE)
                    signal_gen.reset()
                    record_start_t = t
                    signal_start_t = t
                    recording      = True
                ...
                if recording:
                    ...
                    f_des = plotter.params['f_des']   # const slider
...
def main() -> None:
    ...
    logger_box: list = [None]   # logger_box[0] is DataLogger | None
